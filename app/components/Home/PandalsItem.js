...
function PandalsItem({pandals, showDir = false}) {

   const GOOGLE_API_KEY = process.env.NEXT_PUBLIC_GOOGLE_API_KEY;
   const photo_ref = pandals?.photos
     ? pandals?.photos[0]?.photo_reference
     : "";
     const { userLocation, setUserLocation } = useContext(UserLocationContext);
     const [distance, setDistance] = useState();
     useEffect(() => {
       calculateDistance(
         pandals.geometry.location.lat,
         pandals.geometry.location.lng,
         userLocation.lat,
         userLocation.lng
       );
     }, []);
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1 items-center">
        ...
        {/* <h2 className="text-[10px] font-bold">{pandals.rating}</h2> */}
      </div>
      {showDir ? (
